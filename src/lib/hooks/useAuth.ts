...
const sanitizeError = (error: any): string => {
  ...
  if (typeof error === 'string') return error.length > 100 ? 'An error occurred' : error;
  if (error.message) {
...
// SECURITY: Client-side rate limiting with memory management
// WARNING: This can be bypassed - MUST implement server-side rate limiting
...
export function useAuth() {
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    try {
      // NOTE: Supabase JS client doesn't support AbortSignal yet
      // This structure is ready for when they add support
      ...
      // SECURITY: Verify profile ID matches requested user (prevent ID mismatch attacks)
      if (data.id !== userId) {
      ...
      const isCriticalError = error instanceof Error && 
        (error.message === 'Invalid profile data structure' ||
         error.message === 'Profile ID mismatch' ||
         error.message === 'Invalid email in profile' ||
         error.message === 'Invalid full name in profile' ||
  ...
  useEffect(() => {
    const supabase = createClient();
    let initializingRef = { current: true };
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      ...
      // SECURITY: Client-side rate limiting (can be bypassed)
      // IMPORTANT: Implement server-side rate limiting via Supabase auth settings
  ...
  const isOwner = !loading && isActive && profile?.role === 'owner';
  const isAdmin = !loading && isActive && profile?.role === 'admin';
  const isOfficer = !loading && isActive && profile?.role === 'officer';
