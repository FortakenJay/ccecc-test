...
import {faBookOpen, faAward, faMusic, faPalette} from "@fortawesome/free-solid-svg-icons";
            ...
            <section
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center">
                        <h1 className="text-4xl lg:text-5xl mb-4">{t('title')}</h1>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                {hskClasses.length > 0 && (
                    <section className="mb-16">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                                <h2 className="text-gray-900">{t('prepHSK')}</h2>
                                <p className="text-gray-600">{t('prepHSKdesc')}</p>
                            </div>
                        </div>
                                    ...
                                    {classItem.price_colones && (
                                        <div className="text-2xl font-bold text-[#C8102E] mb-6">
                                            ₡{classItem
                                                .price_colones
                                                .toLocaleString()}
                                            <span className="text-sm text-gray-500 font-normal">{t('per')}</span>
                                        </div>
                                    )}
                                    <Link
                ...
                {languageClasses.length > 0 && (
                    <section className="mb-16">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                                <h2 className="text-gray-900">{t('chineseClass')}</h2>
                                <p className="text-gray-600">{t('chineseClassdesc')}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {languageClasses.map((classItem : any) => (
                                <Card
                                    key={classItem.id}
                                    className="p-6 hover:shadow-xl transition-shadow border-t-4 border-t-[#FFD700]">
                                    {classItem.level && (
                                        <div className="mb-4">
                ...
                {culturalClasses.length > 0 && (
                    <section id="artistic">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                                <h2 className="text-gray-900">{t('artisticalExchange')}</h2>
                                <p className="text-gray-600">{t('artisticExchangeDesc')}</p>
                            </div>
                        </div>
                ...
                {talleresClasses.length > 0 && (
                    <section className="mb-16 py-12">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                                <h2 className="text-gray-900">{t('workshop')}</h2>
                                <p className="text-gray-600">E{t('workshopDesc')}</p>
                            </div>
                        </div>
                                ...
                                <Card
                                    ...
                                    {classItem.level && (
                                        <div className="mb-4">
                                            <Badge
                                                variant="outline"
                                                className="text-[#FFA500] border-[#FFA500] text-sm px-2 py-1">
                                                {classItem.level}
                                            </Badge>
                                        </div>
                                    )}
                ...
                {!loading && hskClasses.length === 0 && languageClasses.length === 0 && culturalClasses.length === 0 && talleresClasses.length === 0 && (
                    <div className="text-center py-16">
                        ...
                        <h3 className="text-2xl font-bold text-gray-900 mb-2">{t('noClass')}</h3>
                        <p className="text-gray-600">{t('comeBack')}</p>
                    </div>
                )}
            </div>
        </div>
